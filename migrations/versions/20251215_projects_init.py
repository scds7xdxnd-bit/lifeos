...
def upgrade():
    ...
    if not _has_table("project"):
        ...
        if not _has_column("project", "updated_at"):
            op.add_column("project", sa.Column("updated_at", sa.DateTime(), nullable=True, server_default=sa.func.now()))
    ...
    if not _has_table("project_task"):
        ...
        if not _has_column("project_task", "updated_at"):
            op.add_column("project_task", sa.Column("updated_at", sa.DateTime(), nullable=True, server_default=sa.func.now()))
    ...
    if not _has_table("project_task_log"):
        ...
        if not _has_column("project_task_log", "status_snapshot"):
            op.add_column("project_task_log", sa.Column("status_snapshot", sa.String(length=32), nullable=True))
        if not _has_column("project_task_log", "created_at"):
            op.add_column("project_task_log", sa.Column("created_at", sa.DateTime(), nullable=True, server_default=sa.func.now()))
