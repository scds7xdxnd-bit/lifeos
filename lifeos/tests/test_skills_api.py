...
class TestSkillsAPI:
    ...
    def test_get_skill_path_degrades_gracefully_on_compute_error(self, app, client, test_user, auth_headers):
        """Path endpoint should return a minimal safe path instead of 500 on transient compute failures."""
        app.config["ENABLE_PHASE12_SKILLS_PATH"] = True
        with app.app_context():
            skill = create_skill(test_user.id, name="Path Degrade Skill")

        with patch("lifeos.domains.skills.controllers.skill_api.get_skill_path", side_effect=RuntimeError("boom")):
            resp = client.get(f"/api/skills/{skill.id}/path", headers=auth_headers)
...
class TestPracticeSessionAPI:
    ...
    def test_log_practice_succeeds_when_path_resolution_errors(self, app, client, test_user, csrf_headers):
        """Practice logging should remain successful even if follow-up path resolution throws."""
        app.config["ENABLE_PHASE12_SKILLS_PATH"] = True
        with app.app_context():
            skill = create_skill(test_user.id, name="Practice Path Error Skill")

        with patch("lifeos.domains.skills.controllers.skill_api.get_skill_path", side_effect=RuntimeError("boom")):
            resp = client.post(f"/api/skills/{skill.id}/practice", json={"duration_minutes": 25}, headers=csrf_headers)
