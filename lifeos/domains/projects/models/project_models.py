...
class ProjectTask(db.Model):
    ...
    __table_args__ = (
        db.Index("ix_project_task_user_project_status", "user_id", "project_id", "status"),
        db.Index("ix_project_task_user_due_date", "user_id", "due_date"),
        db.Index("ix_project_task_user_project_due_date", "user_id", "project_id", "due_date"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[int] = mapped_column(db.ForeignKey("user.id"), index=True, nullable=False)
    project_id: Mapped[int] = mapped_column(db.ForeignKey("project.id", ondelete="CASCADE"), index=True, nullable=False)
...
class ProjectTaskLog(db.Model):
    ...
    __table_args__ = (
        db.Index("ix_project_task_log_user_task_logged_at", "user_id", "task_id", "logged_at"),
