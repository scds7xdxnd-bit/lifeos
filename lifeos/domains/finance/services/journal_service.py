...
def record_transaction(user_id: int, amount: float, debit_account_id: int, credit_account_id: int, description: str):
    ...
    return post_journal_entry(user_id=user_id, description=description or f"Transaction {datetime.utcnow().isoformat()}", lines=lines)
