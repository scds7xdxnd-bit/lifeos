...
def trial_balance_view(user_id: int, as_of: dt.date | None = None) -> dict:
    ...
    accounts: List[Account] = (
        ...
        .order_by(
            ...
            Account.category_id.asc().nullsfirst(),
            Account.code.asc().nullsfirst(),
            Account.name.asc(),
        )
        .all()
    )
    rows: List[dict] = []
    category_totals: Dict[tuple[str, int | None], Dict[str, float]] = defaultdict(
        lambda: {"debit": 0.0, "credit": 0.0, "net": 0.0}
    )
