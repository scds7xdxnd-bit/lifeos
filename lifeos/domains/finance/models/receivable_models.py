...
class ReceivableTracker(db.Model):
    ...
    loan_group_links: Mapped[list["LoanGroupLink"]] = relationship(
        "LoanGroupLink", back_populates="tracker"
    )
...
class ReceivableManualEntry(db.Model):
    ...
    tracker_id: Mapped[int] = mapped_column(
        db.ForeignKey("finance_receivable_tracker.id"), index=True, nullable=False
    )
    ...
    tracker: Mapped[ReceivableTracker] = relationship(
        "ReceivableTracker", back_populates="manual_entries"
    )
...
class LoanGroupLink(db.Model):
    ...
    group_id: Mapped[int] = mapped_column(
        db.ForeignKey("finance_loan_group.id"), index=True, nullable=False
    )
    tracker_id: Mapped[int] = mapped_column(
        db.ForeignKey("finance_receivable_tracker.id"), index=True, nullable=False
    )
    ...
    tracker: Mapped[ReceivableTracker] = relationship(
        "ReceivableTracker", back_populates="loan_group_links"
    )
