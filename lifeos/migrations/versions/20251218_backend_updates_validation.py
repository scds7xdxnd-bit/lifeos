...
def upgrade() -> None:
    ...
    if _has_table("finance_account"):
        ...
        if not _has_index("finance_account", "ix_finance_account_user_normalized_name"):
            op.create_index("ix_finance_account_user_normalized_name", "finance_account", ["user_id", "normalized_name"])
    ...
    if not _has_index("finance_journal_entry", "ix_finance_journal_entry_user_posted_at"):
        op.create_index("ix_finance_journal_entry_user_posted_at", "finance_journal_entry", ["user_id", "posted_at"])
    ...
    if not _has_table("finance_transaction"):
        op.create_table(
            ...
            sa.Column("journal_entry_id", sa.Integer(), sa.ForeignKey("finance_journal_entry.id"), nullable=True),
    ...
    if not _has_index("finance_transaction", "ix_finance_transaction_user_occurred_at"):
        op.create_index("ix_finance_transaction_user_occurred_at", "finance_transaction", ["user_id", "occurred_at"])
    ...
    if not _has_index("finance_money_schedule_row", "ix_finance_money_schedule_row_user_event_date"):
        op.create_index("ix_finance_money_schedule_row_user_event_date", "finance_money_schedule_row", ["user_id", "event_date"])
    ...
    if not _has_index("finance_money_schedule_daily_balance", "ix_finance_money_schedule_daily_balance_user_as_of"):
        op.create_index("ix_finance_money_schedule_daily_balance_user_as_of", "finance_money_schedule_daily_balance", ["user_id", "as_of"])
    ...
    if not _has_table("finance_money_schedule_scenario_row"):
        op.create_table(
            ...
            sa.Column("base_row_id", sa.Integer(), sa.ForeignKey("finance_money_schedule_row.id"), nullable=True),
    ...
    if not _has_table("habits_habit_log"):
        op.create_table(
            ...
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("user_id", sa.Integer(), sa.ForeignKey("user.id"), nullable=False, index=True),
            sa.Column("habit_id", sa.Integer(), sa.ForeignKey("habits_habit.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    if not _has_index("habits_habit_log", "ix_habits_log_habit_logged_date"):
        op.create_index("ix_habits_log_habit_logged_date", "habits_habit_log", ["habit_id", "logged_date"])
    ...
    if not _has_table("skill_practice_session"):
        op.create_table(
            ...
            sa.Column("skill_id", sa.Integer(), sa.ForeignKey("skill.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("duration_minutes", sa.Integer(), nullable=False, server_default="0"),
            sa.Column("intensity", sa.Integer(), nullable=True),
            sa.Column("notes", sa.Text(), nullable=True),
            sa.Column("practiced_at", sa.DateTime(), server_default=sa.func.now(), nullable=False, index=True),
            sa.Column("created_at", sa.DateTime(), server_default=sa.func.now(), nullable=False),
        )
    ...
    if not _has_index("project_task", "ix_project_task_user_project_status"):
        op.create_index("ix_project_task_user_project_status", "project_task", ["user_id", "project_id", "status"])
    ...
    if not _has_index("project_task", "ix_project_task_user_project_due_date"):
        op.create_index("ix_project_task_user_project_due_date", "project_task", ["user_id", "project_id", "due_date"])
    ...
    if not _has_table("project_task_log"):
        op.create_table(
            ...
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("user_id", sa.Integer(), sa.ForeignKey("user.id"), nullable=False, index=True),
            sa.Column("task_id", sa.Integer(), sa.ForeignKey("project_task.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("note", sa.Text(), nullable=True),
    ...
    if not _has_index("project_task_log", "ix_project_task_log_user_task_logged_at"):
        op.create_index("ix_project_task_log_user_task_logged_at", "project_task_log", ["user_id", "task_id", "logged_at"])
    if not _has_index("project_task_log", "ix_project_task_log_user_logged_at"):
        op.create_index("ix_project_task_log_user_logged_at", "project_task_log", ["user_id", "logged_at"])
