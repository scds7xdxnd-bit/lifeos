      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
        {t.items.map(({ quote, name, role }) => (
          <Card key={name} style={{ padding: '32px' }}>
            ...
            <p style={{ ...typography.label, color: colors.onSurface, margin: 0, fontSize: '0.92rem' }}>
              {name}
            </p>
            <p style={{ color: colors.outline, margin: '2px 0 0', fontSize: '0.82rem', fontFamily: fonts.sans }}>
              {role}
            </p>
          </Card>
        ))}
